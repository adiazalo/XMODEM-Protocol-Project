...
#define UART_MSR_OFFSET (UART_REG_OFFSET + 0x18) /**< Modem Status Register */
...
{
        kfifo_in(&uart->in_fifo, &data, sizeof(data));
...
{
	iowrite32(data, uart->base_addr + UART_THR_OFFSET); //Transmitter Holding Buffer Register
}

static inline u32 tx_ready(void)
{
	return ioread32(uart->base_addr + UART_LSR_OFFSET) & UART_LSR_TX_BUFFER_EMPTY; //Line Status Register
}

static inline u32 rx_ready(void)
{
	return ioread32(uart->base_addr + UART_LSR_OFFSET) & UART_LSR_DATA_READY; //Line Status Register
}
...
static void uart_tasklet_func(unsigned long d) {
	...
	char uartOutput;
	while (tx_ready() && kfifo_out(&uart->out_fifo,&uartOutput, sizeof(uartOutput))){
		wake_up_interruptible(&uart->writeable);
		uart_send(uartOutput);
		iowrite32(1, uart->base_addr + UART_THR_OFFSET);
	}
                ...
                size_t count, loff_t *ppos)
{
        ...
        if (kfifo_is_empty(&uart->in_fifo)) {
                if (file->f_flags & O_NONBLOCK) {
            			...
            			ret = wait_event_interruptible(uart->readable, !kfifo_is_empty(&uart->in_fifo));
